...
# Check data
cat("delta_Tn range:", range(df$delta_Tn, na.rm = TRUE), "\n")
cat("Missing values in delta_Tn:", sum(is.na(df$delta_Tn)), "\n")
...
model_original <- lmer(delta_Tn ~ treatment * thermal_phase + (1|tube_id) + (1|date_factor), 
                       data = df, REML = FALSE, control = control_opts)

# Create residual diagnostic plots for original model
png("residuals_original_delta_Tn.png", width = 3600, height = 1200, res = 250)
par(mfrow = c(1, 3), 
    mar = c(5, 5, 4, 2),
    cex.main = 2.5,
    cex.lab = 2.2,
...
df$delta_Tn_trans <- sign(df$delta_Tn) * abs(df$delta_Tn)^(1/3)

model_transformed <- lmer(delta_Tn_trans ~ treatment * thermal_phase + (1|tube_id) + (1|date_factor), 
                          data = df, REML = FALSE, control = control_opts)

# Create residual diagnostic plots for transformed model
png("residuals_transformed_delta_Tn.png", width = 3600, height = 1200, res = 250)
par(mfrow = c(1, 3), 
    mar = c(5, 5, 4, 2),
    cex.main = 2.5,
    cex.lab = 2.2,
...
if(shapiro_trans$p.value > shapiro_orig$p.value && shapiro_trans$p.value > 0.001) {
  selected_variable <- "delta_Tn_trans"
  selected_description <- "cube root transformed delta_Tn"
  cat("*** SELECTED: Transformed data (better normality) ***\n")
} else if(shapiro_orig$p.value > 0.05) {
  selected_variable <- "delta_Tn"
  selected_description <- "original delta_Tn"
...
model_comparison <- data.frame(
  ...
  Formula = c("treatment × thermal_phase", "treatment × time_block", "treatment × thermal_phase × time_block"),
...
write.csv(table_1, "Model_1_Treatment_x_Thermal_Phase_deltaTN.csv", row.names = FALSE)
write.csv(table_2, "Model_2_Treatment_x_Time_Block_deltaTN.csv", row.names = FALSE)
write.csv(table_3, "Model_3_Full_Factorial_deltaTN.csv", row.names = FALSE)

# Save model comparison
write.csv(model_comparison, "Model_Comparison_deltaTN.csv", row.names = FALSE)
...
write.csv(treatment_summary, "Treatment_Summary_deltaTN.csv", row.names = FALSE)
...
cat("\nFiles generated:\n")
cat("- residuals_original_delta_Tn.png\n")
cat("- residuals_transformed_delta_Tn.png\n")
cat("- Model_1_Treatment_x_Thermal_Phase_deltaTN.csv\n")
cat("- Model_2_Treatment_x_Time_Block_deltaTN.csv\n")
cat("- Model_3_Full_Factorial_deltaTN.csv\n")
cat("- Model_Comparison_deltaTN.csv\n")
cat("- Treatment_Summary_deltaTN.csv\n")
