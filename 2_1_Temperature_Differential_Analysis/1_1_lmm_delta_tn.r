...
# 3. Combined results output for publication
# ============================================================================
...
min_val <- min(df$delta_Tn, na.rm = TRUE)
shift_val <- abs(min_val) + 0.01

# Create all transformation variables
df$delta_Tn_log <- log(df$delta_Tn + shift_val)
df$delta_Tn_sqrt <- sqrt(df$delta_Tn + shift_val)
df$delta_Tn_cbrt <- sign(df$delta_Tn) * abs(df$delta_Tn)^(1/3)
...
model_log <- lmer(delta_Tn_log ~ treatment * thermal_phase + (1|tube_id) + (1|date_factor), 
                  data = df, REML = FALSE, control = control_opts)
model_sqrt <- lmer(delta_Tn_sqrt ~ treatment * thermal_phase + (1|tube_id) + (1|date_factor), 
                   data = df, REML = FALSE, control = control_opts)
model_cbrt <- lmer(delta_Tn_cbrt ~ treatment * thermal_phase + (1|tube_id) + (1|date_factor), 
                   data = df, REML = FALSE, control = control_opts)
...
if(best_transformation == "Original") {
  ...
  selected_variable <- "delta_Tn_log"
  selected_description <- "log transformed delta_Tn"
  selected_model <- model_log
} else if(best_transformation == "SQRT") {
  selected_variable <- "delta_Tn_sqrt"
  selected_description <- "square root transformed delta_Tn"
  selected_model <- model_sqrt
} else if(best_transformation == "CBRT") {
  selected_variable <- "delta_Tn_cbrt"
...
write.csv(combined_results, "Combined_LMM_Delta_Tn_Complete_Results.csv", row.names = FALSE)
