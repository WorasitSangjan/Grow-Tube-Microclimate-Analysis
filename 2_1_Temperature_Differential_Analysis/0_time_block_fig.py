...
def create_time_block_figure(hourly_stats: pd.DataFrame, weather_df: pd.DataFrame) -> None:
    ...
    line1 = ax1.plot(hourly_plot["hour"], hourly_plot["air_temp_C_mean"], 
                     label="Mean Temp", color="steelblue", linewidth=3)
    line2 = ax1.plot(hourly_plot["hour"], hourly_plot["air_temp_C_min"], 
                     label="Min Temp", color="steelblue", linewidth=2, linestyle='--', alpha=0.7)
    line3 = ax1.plot(hourly_plot["hour"], hourly_plot["air_temp_C_max"], 
                     label="Max Temp", color="steelblue", linewidth=2, linestyle=':', alpha=0.7)
    
    ax1.set_ylabel("Air Temperature (°C)", color="steelblue", fontsize=18)
    ...
    ax2 = ax1.twinx()
    line4 = ax2.plot(hourly_plot["hour"], hourly_plot["solar_radiation_mean"], 
                     label="Mean Radiation", color="darkorange", linewidth=3)
    line5 = ax2.plot(hourly_plot["hour"], hourly_plot["solar_radiation_min"], 
                     label="Min Radiation", color="darkorange", linewidth=2, linestyle='--', alpha=0.7)
    line6 = ax2.plot(hourly_plot["hour"], hourly_plot["solar_radiation_max"], 
                     label="Max Radiation", color="darkorange", linewidth=2, linestyle=':', alpha=0.7)
    
    ax2.set_ylabel("Solar Radiation (W/m²)", color="darkorange", fontsize=18)
    ...
    for divider in time_dividers:
        ax1.axvline(x=divider, color="gray", linestyle="--", linewidth=1.5, alpha=0.8)
    ...
    time_labels = {
        ...
        "Peak Heat | 11-15": 11,
        "Evening Cooling | 15-19": 15,
        "Night Period | 19-07": 19
    }
    for label, hour in time_labels.items():
        ax1.text(hour + 0.25, y_max - 1.5, label, rotation=90, ha="left", va="top",
                 fontsize=18, color="gray", alpha=0.7)
    
    # Legend
    lines = line1 + line2 + line3 + line4 + line5 + line6
    labels = ["Mean Temp", "Min Temp", "Max Temp", "Mean Radiation", "Min Radiation", "Max Radiation"]
    ax1.legend(lines, labels, loc='upper center', bbox_to_anchor=(0.5, -0.125), 
               ncol=6, frameon=False, fontsize=16)
    
    # GRAPH 2: SOLAR RADIATION DISTRIBUTION BY TIME BLOCK
    ax3 = plt.subplot(3, 1, 2)
    
    time_block_order = ["Morning Warming", "Peak Heat", "Evening Cooling", "Night Period"]
    colors = ['khaki', 'lightcoral', 'orange', 'lightblue']
    ...
    solar_violin_parts = ax3.violinplot(solar_violin_data, positions=range(len(time_block_order)), 
    ...
    ax3.axhline(overall_solar_mean, color='red', linestyle='-', linewidth=2,
                label=f"Overall Mean: {overall_solar_mean:.0f} W/m²")
    
    ax3.set_xlabel("Time Block", fontsize=12)
    ax3.set_ylabel("Solar Radiation (W/m²)", fontsize=12)
    ax3.set_title("Solar Radiation Distribution by Time Block", fontsize=14, fontweight='bold')
    ax3.set_xticks(range(len(time_block_order)))
    ax3.set_xticklabels([name.replace(' ', '\n') for name in time_block_order], fontsize=11)
    ...
    temp_violin_parts = ax4.violinplot(temp_violin_data, positions=range(len(time_block_order)), 
                                      showmeans=True, showmedians=True)
    ...
    ax4.axhline(overall_temp_mean, color='red', linestyle='-', linewidth=2, 
                label=f"Overall Mean: {overall_temp_mean:.1f}°C")
    
    ax4.set_xlabel("Time Block", fontsize=12)
    ax4.set_ylabel("Air Temperature (°C)", fontsize=12)
    ax4.set_title("Temperature Distribution by Time Block", fontsize=14, fontweight='bold')
    ax4.set_xticks(range(len(time_block_order)))
