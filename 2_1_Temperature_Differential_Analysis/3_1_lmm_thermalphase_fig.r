...
# Run this after completing your LMM analysis (need emm_1 and back-transformed data)
# ============================================================================
...
cat("Creating temperature effects figure...\n")
...
if(!exists("emm_1")) {
  stop("Error: 'emm_1' object not found. Please run your LMM analysis first.")
}

# Check for back-transformed data file
celsius_file <- "Model_1_BackTransformed_Results.csv"
...
# Clean thermal phase names and set order
clean_thermal_names <- function(x) {
  factor(x, levels = c("Cold Cloudy", "Cold Sunny", "Warm Cloudy", "Warm Sunny"))
}

# Apply cleaning to plot data
plot_data$treatment_clean <- clean_treatment_names(plot_data$treatment)
plot_data$thermal_phase_clean <- clean_thermal_names(plot_data$thermal_phase)

# Apply cleaning to celsius data
celsius_data$treatment_clean <- clean_treatment_names(celsius_data$Treatment)
celsius_data$thermal_phase_clean <- clean_thermal_names(celsius_data$Thermal_Phase)
...
cat("Plot data thermal phases:", unique(plot_data$thermal_phase_clean), "\n")
cat("Celsius data thermal phases:", unique(celsius_data$thermal_phase_clean), "\n")

# Merge datasets
plot_data_final <- merge(
  plot_data, 
  celsius_data[, c("treatment_clean", "thermal_phase_clean", "EMM_degrees_C")],
  by = c("treatment_clean", "thermal_phase_clean"),
  all.x = TRUE
)
...
if (any(is.na(plot_data_final$EMM_degrees_C))) {
  ...
  print(plot_data_final[is.na(plot_data_final$EMM_degrees_C), c("treatment_clean", "thermal_phase_clean")])
}
...
cld_data <- summary(cld(emm_1, Letters = letters, decreasing = TRUE))

# Clean CLD data
cld_data$treatment_clean <- clean_treatment_names(cld_data$treatment)
cld_data$thermal_phase_clean <- clean_thermal_names(cld_data$thermal_phase)
...
p <- ggplot(plot_data_final, aes(x = treatment_clean, y = emmean, fill = EMM_degrees_C)) +
  ...
  # Labels and titles
  labs(
    title = "Grow Tube Temperature Effects by Thermal Phase",
...
ggsave("Temperature_Effects_Figure.png", p, 
       width = 16, height = 4.5, dpi = 300, bg = "white")

cat("Figure saved: Temperature_Effects_Figure.png\n")
...
cat("Four thermal phase panels in single row\n")
